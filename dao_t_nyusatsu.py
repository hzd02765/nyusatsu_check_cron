...
# @param  : なし
# @return : なし
def del_t_nyusatsu(conn, cur):
	sql = "delete from t_nyusatsu"
	util.print_log("SQL : " + sql)
	cur.execute(sql)
	conn.commit()

# 案件情報トランザクションのMaxIDを取得
# @param  : なし
# @return : 案件情報トランザクションのMaxID
def get_max_id(cur):
	sql = u"select max(id) from t_nyusatsu"
	cur.execute(sql)
	record =  cur.fetchone()
